...
class Scene:
    ...
    def error_cam_orig(self,id_cam,thres=0,dist=True,rs_crt=False,motion=False,norm=False,motion_weights=0):
        '''
...
class Scene_multi_spline(Scene):
    ...
    def error_cam(self,cam_id,mode='dist',motion=False,norm=False,motion_weights=0):
        ...
        mot_idx = np.array([]) #np.zeros((self.detecti
        for i in range(interval.shape[1]):
    ...
    def BA(self, numCam, max_iter=10,rs_crt=False,motion=False,motion_weights=0):
        ...
        cam_parts = 2
        if rs_crt:
            for i,j in enumerate(model_parts[cam_parts]):
                self.cameras[self.sequence[i]].rs = j
            cam_parts = 3
...
class Scene_single_spline(Scene):
    ...
    def error_cam_sing(self,cam_id,mode='dist'):
        '''
        Calculate the reprojection errors for a given camera
